...
- 流式/渲染：`agent_message_chunk`、thought 流式、tool 卡片、plan(todo 折叠)、
  ...
  **user-invocable skills**：全局 `~/.agents/skills`、工程 `.agents/skills`/`.dsh/skills`
  ...
  命名沿用 pi-acp 惯例：skill 通告为 `skill:<name>`（弹窗显示 `/skill:find-skills`），
  命令保持原名；列表按命令块 + skill 块分区（Zed 1.18 没有 agent 侧的
  Commands/Skills/Actions 分组）。prompt 收到 `/skill:<name>` 时归一化为裸
  `/name` 手势，交给 dsh tool-skill 装载（与 dsh Web "/" 选 skill 同路径）。
