...
*/
function sessionDirForDelete(artifactPath, sessionsRoot) {
	const base = sessionsRoot.endsWith(sep) ? sessionsRoot : sessionsRoot + sep;
	if (!artifactPath.startsWith(base)) return void 0;
	const dir = dirname(artifactPath);
	return SESSION_DIR_NAME.test(basename(dir)) ? dir : void 0;
}
...
function makeRecord(id, cwd, handle, selection, init = {}) {
	return {
		id,
		cwd,
		agent: handle.agent,
...
*/
function lastAgentPreset(events) {
	for (let index = events.length - 1; index >= 0; index -= 1) {
		const event = events[index];
...
* that condition.
*/
function hasStartedTurn(events) {
...
*/
function configOptionsUpdate(configOptions) {
	return {
...
*/
function sessionInfoUpdate(title) {
	return {
...
*/
function clearStreamKeys(acc, prefix) {
	for (const key of [...acc.keys()]) if (key.startsWith(prefix)) acc.delete(key);
}
/**
...
*/
function foldStreamFrame(state, frame) {
	if (frame.type === "start") {
	...
	if (frame.type === "end") {
		...
		if (frame.outcome.kind === "abandoned" && attempt !== void 0) {
...
*/
function requestPermissionRequest(sessionId, toolCallId) {
	return {
		sessionId,
		toolCall: { toolCallId },
		options: [
...
*/
function elicitationRequestFor(request, sessionId, toolCallId) {
	const properties = {};
	const required = [];
	const messages = [];
...
const ELICITATION_UNSUPPORTED = "ELICITATION_UNSUPPORTED";
const ELICITATION_NO_SESSION = "ELICITATION_NO_SESSION";
const ELICITATION_ABORTED = "ELICITATION_ABORTED";
const ELICITATION_DECLINED = "ELICITATION_DECLINED";
const ELICITATION_CANCELLED = "ELICITATION_CANCELLED";
/** Prefix a model-facing failure message with its stable code. */
const codedError = (code, detail) => /* @__PURE__ */ new Error(`[${code}] ${detail}`);
//#endregion
//#region src/bridge/elicitation.ts
...
const askViaForm = async (request, bridge) => {
	const agent = request.agent;
	const conn = bridge.getConn();
	const record = agent !== void 0 ? bridge.store.get(agent.session.id) : void 0;
	if (record === void 0 || record.closed || conn === void 0) throw codedError(ELICITATION_NO_SESSION, "no live ACP session for this question");
	if (!bridge.formsEnabled()) throw codedError(ELICITATION_UNSUPPORTED, "this ACP client does not support elicitation forms; answer the question inline instead");
	const signal = request.signal;
	if (signal !== void 0 && signal.aborted) throw codedError(ELICITATION_ABORTED, "question aborted");
...
* not unsupported).
*/
function thoughtLevelCurrentFor(current, reasoning) {
	const pick = current ?? reasoning?.defaultEffort ?? "provider-default";
	if (thoughtLevelOptionOptions(reasoning).some((effort) => effort.id === pick)) return pick;
	return reasoning?.defaultEffort ?? "provider-default";
}
/**
...
function thoughtLevelOptionOptions(reasoning) {
	const efforts = reasoning !== void 0 ? reasoning.efforts : CANONICAL_REASONING_LEVELS;
...
*/
const PRESET_LOCKED_DETAIL = "the agent preset is fixed once the session has started; open a new session to change it";
/**
...
*/
function presetChangeFailureDetail(error) {
...
* memory.
*/
function presetSelectOptionList(rows, current) {
	const usable = rows.filter((row) => row.broken === void 0);
	if (usable.length === 0) return null;
	const options = usable.map((row) => ({
		value: row.id,
		name: row.name !== void 0 && row.name !== row.id ? `${row.id} (${row.name})` : row.id,
		description: row.description ?? null
	}));
	return {
		options,
		currentValue: current !== void 0 && options.some((option) => option.value === current) ? current : options[0].value
	};
}
/**
...
var PresetDefaultError = class extends Error {};
...
function encodedImages(prompt) {
	return prompt.flatMap((block) => block.type === "image" ? [{
		type: "image",
...
function apply(ctx, config = {}) {
	...
	*/
	const rosterRows = async () => {
		if (presets === void 0 || presets.list === void 0) return void 0;
		try {
	...
	const availablePresetIds = async () => {
		return (await rosterRows())?.map((row) => row.id).join(", ");
	};
	...
	* absent roster (no preset seats at all) still degrades. */
	const strictDefaultPresetId = async () => {
		if (presets === void 0) return void 0;
		try {
			return (await presets.resolve(presetIdForEnv()))?.id;
		} catch (error) {
			const message = `${presetIdForEnv() !== void 0 ? "DSH_ACP_PRESET preset" : "default agent preset"} unavailable: ${errorChain(error)}`;
			const available = await availablePresetIds();
			throw new PresetDefaultError(available === void 0 ? message : `${message} — available: ${available}`);
		}
	};
	...
	*/
	const deliverStreamFrame = (record, frame) => {
		const live = foldStreamFrame({
			attempts: record.streamAttempts,
			text: record.streamedText,
			reasoning: record.streamedReasoning
		}, frame);
	...
	*/
	const deliverSessionTitle = (record, title) => {
		...
		serialize(record, async () => {
			...
			await notify({
				sessionId: record.id,
				update: sessionInfoUpdate(title)
			});
		});
	};
	...
	*/
	...
	const snapshotMounts = async (record) => {
		if (process.env[ENV_MOUNT_SNAPSHOT] === void 0) return;
		try {
			const toolNames = [...ctx.get("tools")?.schemas(record.agent) ?? []].map((tool) => tool.name).sort();
			const commandNames = [...commands?.list(record.agent) ?? []].map((command) => command.name).sort();
			const dir = join(resolveDshHome(), "snapshots");
			mkdirSync(dir, { recursive: true });
			writeFileSync(join(dir, `${record.id}.json`), JSON.stringify({
				sessionId: record.id,
				tools: toolNames,
				slash: commandNames
			}, null, 2));
	...
	ctx.on("agent/assistant-stream", ({ agent, frame }) => {
		...
		if (record === void 0 || record.agent !== agent) return;
	...
	ctx.on("approval/request", (request, next) => {
		...
		if (record === void 0 || record.agent !== request.agent) return next();
		if (record.allowedTools.has(request.toolName)) return Promise.resolve("allowed-once");
		if (request.callId === void 0) return next();
		const callId = request.callId;
		return drainRecord(record).then(() => conn.requestPermission(requestPermissionRequest(record.id, callId))).then(({ outcome }) => {
		...
		for (const record of store.values()) {
			...
			serialize(record, async () => {
				try {
					await notify({
						sessionId: record.id,
						update: configOptionsUpdate(await refreshConfigOptions(record))
					});
				} catch (error) {
	...
	* patch row's defaults; per-session config options still override both. */
	const routeDefaults = () => {
		...
		if (provider !== void 0 && model !== void 0) selection.current = {
			provider,
			model
		};
		return {
			selection,
			agentOptions: provider !== void 0 || model !== void 0 ? {
				provider,
				model
			} : void 0
		};
	...
	*/
	const configOptionsCache = /* @__PURE__ */ new WeakMap();
	const refreshConfigOptions = async (record) => {
		const out = [];
		if (!record.turnStarted && presets?.list !== void 0 && presets.select !== void 0) {
			const rows = await rosterRows();
			const preset = rows !== void 0 ? presetSelectOptionList(rows, record.agentPreset) : null;
			if (preset !== null) out.push({
				type: "select",
				id: CONFIG_ID_PRESET,
				name: "Preset",
		...
		if (llm !== void 0 && current !== void 0 && current.provider !== void 0 && current.model !== void 0) {
			try {
			...
			record.supportedEfforts = reasoning !== void 0 ? new Set((reasoning.efforts ?? []).map((effort) => effort.id)) : void 0;
	...
	*/
	const dropPresetOption = (record) => {
		if (presets?.list === void 0 || presets.select === void 0) return;
		const cached = configOptionsCache.get(record);
		if (cached === void 0 || !cached.some((option) => option.id === CONFIG_ID_PRESET)) return;
		...
		serialize(record, async () => {
			...
			try {
				await notify({
					sessionId: record.id,
					update: configOptionsUpdate(next)
				});
			} catch (error) {
	...
	* plugin events).
	*/
	const setSelection = (record, next) => {
	...
	const applyConfigOption = async (record, configId, value) => {
		...
		if (configId === CONFIG_ID_PRESET) {
			if (presets?.select === void 0) throw invalidParams("agent presets are not mounted");
			if (record.turnStarted) throw invalidParams(PRESET_LOCKED_DETAIL);
			const rows = await rosterRows();
			const offered = rows !== void 0 ? presetSelectOptionList(rows, record.agentPreset) : null;
			if (offered === null || !offered.options.some((option) => option.value === value)) throw invalidParams(`unknown agent preset: ${value}`);
			try {
	...
	*/
	...
	const validateWorkspaceParams = (params) => {
		...
		if (params.mcpServers !== void 0 && params.mcpServers !== null && params.mcpServers.length > 0) logger.warn(`dsh-acp-v1: mcpServers accepted but ignored (${params.mcpServers.length} server(s)); no MCP tools are mounted`);
	};
	...
	*/
	const deletePersisted = async (id) => {
	...
	const trackedImplementation = new Proxy({
		...
		async prepareHistoryResume(params) {
			...
			const snapshot = query !== void 0 ? await query.readSession(sessionId) : void 0;
			...
			const restored = snapshot !== void 0 ? lastModelSelection(snapshot.events) : void 0;
			if (restored !== void 0) selection.current = restored;
			const configOptions = await registerRecord(record, params.replay && snapshot !== void 0 ? async () => {
				await replayHistory(record, snapshot.events);
			} : void 0);
			const restoredTitle = snapshot !== void 0 ? lastSessionTitle(snapshot.events) : void 0;
			if (restoredTitle !== void 0) deliverSessionTitle(record, restoredTitle);
			return { configOptions };
		},
	...
	let quietTimer;
	const tryQuietClose = () => {
		if (!stdinEnded || quietTimer !== void 0) return;
		quietTimer = setTimeout(() => {
			quietTimer = void 0;
			if (!stdinEnded) return;
			if (activeRequests.size > 0) {
				Promise.allSettled([...activeRequests]).then(tryQuietClose);
				return;
			}
			inboundWriter.close().catch(() => void 0);
		}, STDIN_QUIET_MS);
	};
	(async () => {
		for await (const chunk of process.stdin) await inboundWriter.write(chunk);
		stdinEnded = true;
		tryQuietClose();
	})().catch((error) => {
	...
	}, ndJsonStream(Writable.toWeb(process.stdout), inbound.readable));
	let quiescing;
	const quiesce = () => {
