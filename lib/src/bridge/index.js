import { n as resolveDshHome } from "../../lib-CM4_mN7v.js";
...
//#region node_modules/.pnpm/@deepseek-ai+dsh-attachment@0.1.1-rc.2_@deepseek-ai+cordis@4.0.1_@deepseek-ai+dsh-brand_196914f91d44cfdaf1ac8baf85d27e79/node_modules/@deepseek-ai/dsh-attachment/lib/index.js
/** Runtime membership for structurally compatible errors crossing package boundaries. */
const IMAGE_ADMISSION_ERROR_CODE_SET = /* @__PURE__ */ new Set([
	"TOO_MANY_IMAGES",
	"IMAGES_TOO_LARGE",
...
function turnEndToStopReason(kind) {
	switch (kind) {
		case "completed":
		case "aborted":
		case "blocked":
...
function sessionNotification(sessionId, update) {
	return {
		sessionId,
		update
	};
...
function streamTextDelta(acc, key, text, chunk) {
	...
	acc.set(key, next);
	if (next.length === 0 || !next.startsWith(prior)) return void 0;
...
*/
function toolCallContent(text, maxChars = 8e3) {
...
function toolCallTitle(kind, name, rawInput) {
	if (kind === "execute") {
		const command = commandOf(rawInput)?.trim();
		if (command !== void 0 && command.length > 0) return command.length <= 400 ? command : `${command.slice(0, 400)}…`;
	}
	return name;
}
//#endregion
...
*/
function replayUpdatesForEvent(event) {
	switch (event.type) {
		...
		case "tool/result": {
			...
			return [{
				...
				toolCallId: callId,
				status: event.data.error !== void 0 ? "failed" : "completed",
...
* fabricates frames.
*/
function replayPlanFold(events) {
...
function effortOptionsFor(reasoning) {
	...
	if (reasoning?.defaultEffort !== void 0) return void 0;
...
function apply(ctx, config = {}) {
		...
		serialize(record, async () => {
			...
			for (let index = 0; index < blocks.length; index += 1) {
				...
				if (block.type === "text") {
					...
					if (remainder !== void 0) await notify(sessionNotification(record.id, assistantTextChunk(remainder)));
				} else if (block.type === "reasoning") {
					const remainder = committedBlockRemainder(record.streamedReasoning, key, block.text);
					if (remainder !== void 0) await notify(sessionNotification(record.id, assistantThoughtChunk(remainder)));
				}
			}
			pushUsage(record);
		});
	...
	*/
	...
	const refreshConfigOptions = async (record) => {
		...
		if (offered.length > 0) {
			...
			const currentEffort = currentEffortFor(current.reasoningEffort !== void 0 ? String(current.reasoningEffort) : void 0, reasoning?.defaultEffort !== void 0 ? String(reasoning.defaultEffort) : void 0);
			out.push({
				type: "select",
	...
	const guardCurrentEffort = (record) => {
		...
		if (guardReasoningEffort({ reasoningEffort: String(current.reasoningEffort) }, supported).reasoningEffort === void 0) {
			const { reasoningEffort: _stripped, ...rest } = current;
			record.selection.current = rest;
		}
	};
	...
	*/
	const replayHistory = async (record, events) => {
		...
		try {
			for (const event of events) {
				if (record.closed) return;
				for (const update of replayUpdatesForEvent(event)) await notify(sessionNotification(record.id, update));
			}
			const plan = replayPlanFold(events);
			if (plan !== void 0 && !record.closed) await notify(sessionNotification(record.id, plan));
		} finally {
			record.replaying = false;
		}
	};
	/** One ordered "create-or-resume" agent handle for history loads. */
	const resumeAgentFor = async (sessionId, cwd, selection, agentPreset) => {
		const defaults = defaultSelection();
		if (defaults.provider !== void 0 && defaults.model !== void 0) selection.current = {
			provider: defaults.provider,
			model: defaults.model
		};
		let presetId = agentPreset;
		if (presetId === void 0) try {
			presetId = presets?.resolve()?.id;
		} catch (error) {
			logger.warn(`dsh-acp-interactive: default agent preset unavailable: ${errorChain(error)}`);
		}
		const agentOptions = defaults.provider !== void 0 || defaults.model !== void 0 ? {
			provider: defaults.provider,
			model: defaults.model
		} : void 0;
		return {
	...
	if (userQuestions !== void 0) {
		const disposer = userQuestions.registerProvider({ ask: async (request) => {
